...
function toggleSize() {
  if (defaultView == true) {
    ...
    d3.selectAll('.circle').attr('r', function (d) {
      return d.weekend * range / max + baseSize;
    });
    defaultView = false;
    simulation.force("collide", d3.forceCollide(function (d) {
      return d.weekend * range / max + baseSize + 1;
    }));
  } else {
    ...
    d3.selectAll('.circle').attr('r', function (d) {
      return baseSize + d.sales / 3;
    });
    defaultView = true;
    simulation.force("collide", d3.forceCollide(function (d) {
      return d.sales / 3 + baseSize + 1;
    }));
  }
}
...
$.get('https://cors-anywhere.herokuapp.com/http://www.imdb.com/chart/boxoffice', function (data) {
  ...
  titles = rawTitles.map(function (title) {
    return title.slice(2, -1);
  });

  rawLinks = rawData.match(/href="\/title.*?"\n>/g);
  links = rawLinks.map(function (link) {
    return "http://www.imdb.com/" + link.slice(6, -3);
  });
  ...
  sales = rawSales.map(function (sale) {
    return parseFloat(sale.slice(1, -2));
  });

  rawWeekend = rawData.match(/ \$.*?M/g);
  weekend = rawWeekend.map(function (sale) {
    return parseFloat(sale.slice(2, -1));
  });
  ...
  // helpers
  var hoverText = d3.select("body").append("div").attr("class", "hover").text("hoverText");
  ...
  //d3 function
  var svg = d3.select("body").append("svg").attr("id", "chart").attr("width", width).attr("height", height).attr('fill', 'blue');

  // image mapping helpers
  var defs = svg.append("defs");

  defs.selectAll(".poster-art").data(dataCollection).enter().append("pattern").attr("class", "poster-art").attr("id", function (d) {
    return d.rank;
  }).attr("height", "100%").attr("width", "100%").attr("patternContentUnits", "objectBoundingBox").append("image").attr("height", 1.5).attr("width", 1).attr("preserveAspectRatio", "none").attr("xlink:href", function (d) {
    return d.imageUrls;
  });

  // simulation
  simulation = d3.forceSimulation().force("x", d3.forceX(center.x).strength(forceStrength)).force("y", d3.forceY(center.y).strength(forceStrength)).force("collide", d3.forceCollide(function (d) {
    return d.sales / 3 + baseSize + 1;
  })).on("tick", ticked); //KEY LINE FOR ACTIATION

  // draw circles
  var circles = svg.selectAll(".circle") //circle element
  .data(dataCollection).enter().append("circle").attr("class", "circle").attr("id", function (d) {
    return d.rank;
  }).attr("cx", function (d) {
    return center.x;
  }) // positions!
  .attr("cy", function (d) {
    ...
    return "url(#" + d.rank + ")";
  }) // fill content target id of rank
  .on("mouseover", function (d, i) {
    //mouseover hoverText
    hoverText.html("Name: " + d.titles + "<br/>\n         Gross Sales: $" + d.sales + "M<br/>\n         Weekend Sales: $" + d.weekend + "M<br/>\n         Hotness Rank: " + d.rank + " (based on weekend)<br/>");
    hoverText.style("visibility", "visible");
  }).on("mousemove", function () {
    return hoverText.style("top", d3.event.pageY + 10 + "px").style("left", d3.event.pageX + 10 + "px");
  }).on("mouseout", function () {
    return hoverText.style("visibility", "hidden");
  }).call(d3.drag().on("start", dragStart).on("drag", dragged).on("end", dragOver)).on('click', function (d) {
    if (d3.event.defaultPrevented) return;
    window.open(d.links, '_blank');
  }).attr('stroke', 'white').attr('stroke-width', '1px');

  // nodes and ticked
  simulation.nodes(dataCollection).on('tick', ticked);
